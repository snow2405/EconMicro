...
def PayoffMatrices(v1, v2, F):
    A = np.zeros((101, 101), dtype=float)  # A[b1, b2]
    B = np.zeros((101, 101), dtype=float)  # B[b2, b1]
...
def NashEquilibrium(A, B, v1, v2, F):
    br1 = [set() for _ in BIDS]  # br1[b2] = set of best-response b1
    br2 = [set() for _ in BIDS]  # br2[b1] = set of best-response b2
